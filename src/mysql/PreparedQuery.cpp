...
PreparedQuery::PreparedQuery(const std::weak_ptr<Database>& dbase, std::string query) : Query(dbase, std::move(query)) {
    putNewParameters();
}
...
void PreparedQuery::setNumber(unsigned int index, double value) {
	...
	m_parameters.back()[index] = std::shared_ptr<PreparedQueryField>(new TypedQueryField<double>(index, MYSQL_TYPE_DOUBLE, value));
}
...
void PreparedQuery::setString(unsigned int index, const std::string& value) {
	...
	m_parameters.back()[index] = std::shared_ptr<PreparedQueryField>(new TypedQueryField<std::string>(index, MYSQL_TYPE_STRING, value));
}
...
void PreparedQuery::setBoolean(unsigned int index, bool value) {
	...
	m_parameters.back()[index] = std::shared_ptr<PreparedQueryField>(new TypedQueryField<bool>(index, MYSQL_TYPE_BIT, value));
}
...
*/
void PreparedQuery::executeQuery(Database &database, MYSQL* connection, std::shared_ptr<IQueryData> ptr) {

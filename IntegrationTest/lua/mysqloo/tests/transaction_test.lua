...
TestFramework:RegisterTest("[Transaction] should reject queries from a different database", function(test)
	local db = TestFramework:ConnectToDatabase()
	local otherDb = TestFramework:ConnectToDatabase()
	local transaction = db:createTransaction()
	local ownQuery = db:query("SELECT 1")
	local foreignQuery = otherDb:query("SELECT 1")
	local foreignPrepared = otherDb:prepare("SELECT ?")
	transaction:addQuery(ownQuery)
	local status, err = pcall(transaction.addQuery, transaction, foreignQuery)
	test:shouldBeEqual(status, false)
	test:shouldBeEqual(tostring(err):find("different database") != nil, true)
	status, err = pcall(transaction.addQuery, transaction, foreignPrepared)
	test:shouldBeEqual(status, false)
	test:shouldHaveLength(transaction:getQueries(), 1)
	test:Complete()
end)

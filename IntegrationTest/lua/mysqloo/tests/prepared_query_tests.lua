...
TestFramework:RegisterTest("[Prepared Query] null in one parameter set should not affect the next", function(test)
	local db = TestFramework:ConnectToDatabase()
	local qu = db:prepare("SELECT ? as a")
	qu:setNull(1)
	qu:putNewParameters()
	...
	qu:start()
	qu:wait()
	test:shouldBeEqual(qu:getData()[1].a, nil)
	qu:getNextResults()
	test:shouldBeEqual(qu:getData()[1].a, 2)
	qu:getNextResults()
	test:shouldBeEqual(qu:getData()[1].a, "x")
	test:shouldBeEqual(qu:hasMoreResults(), true)
	qu:getNextResults()
	test:shouldBeEqual(qu:hasMoreResults(), false)
	test:Complete()
